...
class TestSet:
    ...
    def display(self, truncate: bool = False, max_lines: int = 3) -> None:
        ...
        print(f"\n{'=' * 80}")
        print(f"TEST CASES SUMMARY ({len(self.test_cases)} cases)")
        print(f"{'=' * 80}")
        ...
        for i, test_case in enumerate(self.test_cases, 1):
            ...
            if test_case.interactions:
                ...
                print(
                    f"│ Jailbreak Target: {context.jailbreak_response_description}"
                )
            ...
            for j, interaction in enumerate(test_case.interactions, 1):
                ...
                if truncate and len(question_lines) > max_lines:
                    for line in question_lines[:max_lines]:
                        print(f"  │   {line}")
                    print(
                        f"  │   ... ({len(question_lines) - max_lines} more lines)"
                    )
                else:
                ...
                if truncate and len(response_lines) > max_lines:
                    for line in response_lines[:max_lines]:
                        print(f"  │   {line}")
                    print(
                        f"  │   ... ({len(response_lines) - max_lines} more lines)"
                    )
                else:
